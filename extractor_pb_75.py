...
FLUX_RATIO_MIN = 0.80                 # Base tolerance (at catalog frequency)
FLUX_RATIO_MAX = 1.20                 # Base tolerance (at catalog frequency)                 
VLSS_MAX_SIZE_DEG = 75.0 / 3600.0
NVSS_MAX_SIZE_DEG = 45.0 / 3600.0
LWA_SIZE_TOLERANCE = 1.20 
WARP_ANCHOR_THRESH_PIX = 10.0        # BDSF pixel threshold for warp anchor extraction
...
class BeamModel:
    def __init__(self, h5_path):
        ...
        if not os.path.exists(self.path):
            print(f"[CRITICAL] Beam file not found at: {self.path}")
            sys.exit(1)

    def load_data(self):
        print(f"[Beam] Loading model from {self.path}...")
...
def parse_vlssr_text(path):
    print("  [VLSSr] Parsing as custom text format...")
    ...
    if not coords:
        print("  [VLSSr] Failed to parse any sources.")
        return None
...
def load_catalog(path, name):
    print(f"[{name}] Loading catalog {path}...")
...
def generate_warp_screens(img_sources, cat_data, wcs, shape, freq_mhz, cat_freq_mhz, lwa_bmaj_deg, clip_sigma, base_name=""):
    print("\n[Ionosphere] Matching Sources...")
    
    # 0. Setup Diagnostic Directory
    diag_dir = "Dewarp_Diagnostics"
    if not os.path.exists(diag_dir): os.makedirs(diag_dir, exist_ok=True)
    ...
    # 4. Flux Check (with frequency-dependent tolerance)
    ...
    # Widen flux tolerance for subbands far from catalog frequency.
    # At the catalog frequency (e.g. 74 MHz for VLSSr), tolerance stays at base ±20%.
    # Further away, spectral index scatter, curvature, and scintillation cause more
    # spread, so we scale the window. The positional match (5 arcmin) is the primary
    # guard against false cross-matches; this flux filter is a secondary sanity check.
    ...
    n_diag = np.sum(mask_pos)
    
    print(f"  [Flux Tolerance] freq={freq_mhz:.1f} MHz, cat={cat_freq_mhz:.0f} MHz, "
          f"alpha={DEFAULT_ALPHA}, tolerance_factor={tolerance_factor:.2f} "
          f"-> ratio window [{eff_flux_min:.2f}, {eff_flux_max:.2f}]")
    print(f"  [Result] Warp Anchors (Flux-Filtered): {n_warp}")
    print(f"  [Result] Diagnostic Matches (Pos Only): {n_diag}")
...
def apply_warp(data, screen_x, screen_y):
    ...
    if (ny != sy) or (nx != sx):
        print(f"  [Error] Warp Mismatch: Image {data.shape} != Screen {screen_x.shape}. Skipping.")
        return None
    grid_y, grid_x = np.mgrid[0:ny, 0:nx]
...
def fit_primary_beam(anchors, obs_date, freq_mhz, base_name, beam_model):
    print("\n[BeamFit] Generating Diagnostics from ALL SPATIAL MATCHES...")
    ...
    plt.title(f"Flux Comparison (Spatial Matches)")
    ...
    plt.ylim(0, 3.0) 
    plt.legend()
    plt.title(f"Beam Diagnostic (Spatial Matches)")
...
def run_pipeline(args):
    base_raw = os.path.splitext(os.path.basename(args.image_i))[0]
    base_name = base_raw.replace('-image', '') if base_raw.endswith('-image') else base_raw
    
    with fits.open(args.image_i) as hdul:
        header = hdul[0].header
        data_i = hdul[0].data.squeeze().astype(np.float32)
        wcs = WCS(header).celestial
        freq_hz = header.get('CRVAL3', 50e6)
        freq_mhz = freq_hz / 1e6
        obs_date = header.get('DATE-OBS')
        if 'BMAJ' in header: bmaj_deg = header['BMAJ']
        else: bmaj_deg = 5.0 / 60.0

    print(f"--- Processing {freq_mhz:.2f} MHz (PB Corrected) ---")

    print(f"[BDSF] Extracting sources ({WARP_ANCHOR_THRESH_PIX:.0f}sig for warp anchors)...")
    img_i_bdsf = bdsf.process_image(args.image_i, thresh_pix=WARP_ANCHOR_THRESH_PIX, thresh_isl=5.0, adaptive_rms_box=True, quiet=True)
    
    sources_raw = []
    for s in img_i_bdsf.sources:
        if not np.isnan(s.posn_sky_max[0]):
            sources_raw.append({
                'ra': s.posn_sky_max[0], 'dec': s.posn_sky_max[1], 
                'flux_peak_I_app': s.peak_flux_max,
                'maj': getattr(s, 'maj_axis', 0.0), 'min': getattr(s, 'min_axis', 0.0)
            })
    df_raw = pd.DataFrame(sources_raw)
    print(f"  [BDSF] Found {len(df_raw)} sources.")
    
    if args.nvss: ref_cat, ref_freq = load_catalog(args.nvss, "NVSS"), 1400.0
    elif args.vlssr: ref_cat, ref_freq = load_catalog(args.vlssr, "VLSSr"), 74.0
    else: ref_cat = None

    if not ref_cat:
        print("Error: Reference catalog required."); sys.exit(1)

    print_calibrator_table(df_raw, ref_cat, obs_date, freq_mhz)

    if args.warp:
        sx, sy, anchors_diag, raw_coords = generate_warp_screens(df_raw, ref_cat, wcs, data_i.shape, freq_mhz, ref_freq, bmaj_deg, args.clip_sigma, base_name=base_name)
        
        if sx is not None:
            # Change to _dewarped.fits
            fits.writeto(f"{base_name}_dewarped.fits", apply_warp(data_i, sx, sy), header, overwrite=True)
            
            if args.image_v and os.path.exists(args.image_v):
                base_v = os.path.splitext(os.path.basename(args.image_v))[0].replace('-image', '')
                with fits.open(args.image_v) as vh:
                    v_data = vh[0].data.squeeze().astype(np.float32)
                    wv = apply_warp(v_data, sx, sy)
                    if wv is not None: fits.writeto(f"{base_v}_dewarped.fits", wv, vh[0].header, overwrite=True)
            
            if args.fit_beam and anchors_diag is not None:
                beam = BeamModel(BEAM_PATH)
                fit_primary_beam(anchors_diag, obs_date, freq_mhz, base_name, beam)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--image_i", required=True)
    parser.add_argument("--image_v")
    parser.add_argument("--nvss")
    parser.add_argument("--vlssr")
    parser.add_argument("--warp", action="store_true")
    parser.add_argument("--fit_beam", action="store_true")
