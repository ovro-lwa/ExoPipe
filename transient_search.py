#!/usr/bin/env python3
"""
transient_search.py — OVRO-LWA Transient Search Module

Stokes V: Search each image directly (NO reference subtraction).
  - Cutout: I + V side by side (2x2 deg), reticle + measured flux.
  - Debug: debug_rms_V_{basename}.fits, debug_mask_V_{basename}.fits only.

Stokes I: Subtract deep from each 10min snapshot.
  - Cutout: I reference + subtracted I side by side (2x2 deg), reticle + measured flux.
  - Debug: debug_rms_I_{basename}.fits, debug_diff_I_{basename}.fits, debug_mask_I_{basename}.fits.

run_test() returns a list of detection dicts (with 'snr' key) so the caller
can sort, filter, and apply a quality gate (top 10 highest-SNR).
"""
import argparse
import os
import sys
...
CONFIG_I = {
    ...
    # Multi-tier masking: [sigma_threshold, dilation_radius_in_beams]
    # Tier 1: Very bright sources (>100σ) — mask 2 beams out
    # Tier 2: Moderate sources (>30σ) — mask 1 beam out
    'MASK_TIERS': [
        [100.0, 2.0],
        [30.0,  1.0],
    ],
...
CONFIG_V = {
    ...
    # Multi-tier masking: [sigma_threshold_in_I, dilation_radius_in_beams]
    # Tier 1: Very bright I sources (>500σ) — mask 2 beams out
    # Tier 2: Bright I sources (>100σ) — mask 1 beam out
    'MASK_TIERS': [
        [500.0, 2.0],
        [100.0, 1.0],
    ],
...
# Cutout size for detection thumbnails
CUTOUT_SIZE = 2.0 * u.deg
MAX_CUTOUTS = 5  # Per call (i.e. per subband, per Stokes)


def _truncated_jname(coord):
    """Generate truncated J-name: J0553+31 format (4-char RA + 3-char Dec).
    Nearby transients share the same directory."""
    ...
    # ra_str like '055311', dec_str like '+312005'
    # Truncate to HHmm and ±DD
    ra_short = ra_str[:4]
    dec_short = dec_str[:3]
...
def _target_basename(path):
    """Extract a unique basename from the target image path (without extension).
    Used to make debug FITS filenames unique per target image."""
...
def calculate_local_rms_map(data, mask, cfg):
    """Compute a local RMS map using boxcar MAD, with NaN interpolation and floor enforcement."""
...
def extract_peaks_flex(data, mask, threshold_sigma, rms_map, wcs, obs_time):
    """Extract peaks above (or below) a sigma threshold, return candidate dicts."""
...
def filter_bilobed_artifacts(candidates, partners, cfg):
    """Remove candidates that have a matching opposite-sign partner (PSF sidelobe artifacts)."""
    clean = []
    artifacts = []
    if not partners:
        return candidates, []
...
def _make_cutout_header(original_header, cutout_wcs, candidate, stokes, basename_tag):
    """Build a cutout FITS header with full metadata from the parent image."""
    h = original_header.copy()
    # Remove higher-dimensional axes
...
def _make_cutout_v(candidate, data_v, wcs_v, data_i, wcs_i, header_v, header_i, out_dir, basename_tag):
    ...
    # Full J-name for unique filenames
    ra_str = coord.ra.to_string(unit=u.hour, sep='', precision=0, pad=True)
    dec_str = coord.dec.to_string(unit=u.deg, sep='', precision=0, alwayssign=True, pad=True)
    full_jname = f"J{ra_str}{dec_str}".replace(' ', '')
    # Truncated J-name for directory grouping
...
def run_test(ref_path, tgt_path, ref_i_path, catalog_path, output_dir=None, mode=None):
    ...
    Args:
        ...
        ref_i_path:   Reference Stokes I image for bright-source masking.
                      For Stokes I, this is the same as ref_path.
                      For Stokes V, pass the V image itself (or an I image if available).
        catalog_path: Path to source catalog CSV.
        output_dir:   Directory for cutouts and debug FITS. If None, derived from tgt_path
                      parent as {work_dir}/detections/.
        mode:         Force 'V' or 'I' mode. If None, auto-detected from path.

    Returns:
        List of detection dicts, each containing:
            coord, flux, snr, el, match_info, cutout_path, stokes, tgt_image
        Sorted by descending |SNR|. Empty list if no detections.
    """
    # DETERMINE MODE
    if mode is not None:
        is_v = mode.upper() == 'V'
    else:
        # Auto-detect: check if 'V' is a directory component
    ...
    # Unique tag for debug filenames based on the target image
    ...
    # --- Load reference (Stokes I subtraction only) ---
    d_ref_sub = None
    h_ref_file = None
    ...
    # --- Build search map ---
    if is_v:
        # Stokes V: NO subtraction — search V image directly
        search_map = d_tgt
        ...
        # Stokes I: subtract deep from snapshot
    ...
    # Calculate Search RMS EARLY (Robust)
    ...
    if is_v:
        # Stokes V: both positive and negative are astrophysically meaningful (LCP/RCP)
        neg_cands_high = extract_peaks_flex(search_map, final_mask, -cand_sig, rms_map, wcs_tgt, time_tgt_mid)
    else:
        # Stokes I: only positive transients are physical
        neg_cands_high = []
    ...
    # --- Build detection list with match info ---
    detections = []
    for c in all_real:
        if is_v:
    ...
    print(f"{'Type':<30} | {'RA (deg)':>9} {'Dec (deg)':>9} | {'RA (hms)':>12} {'Dec (dms)':>12} | {'El':>5} | {'Flux':>8} | {'SNR':>5}")
    print("-" * 110)
    ...
    for det in detections:
        ...
        ra = c.ra.deg
        dec = c.dec.deg
        ra_hms = c.ra.to_string(unit=u.hour, sep=':', precision=1, pad=True)
        dec_dms = c.dec.to_string(unit=u.deg, sep=':', precision=1, alwayssign=True, pad=True)
        print(f"{det['match_info']:<30} | {ra:>9.3f} {dec:>9.3f} | {ra_hms:>12} {dec_dms:>12} | {det['el']:>5.1f} | {det['flux']:>6.3f} Jy | {det['snr']:>5.1f}")

    # --- Generate cutouts (top MAX_CUTOUTS only) ---
    # Determine output directory: detections/ under the work_dir
    if output_dir is None:
        # Walk up from tgt_path (e.g., .../V/deep/image.fits) to the work_dir level
        # work_dir is typically 2 levels up from the image (pol/category/file)
    ...
    if is_v:
        # For V cutouts, we also need the corresponding I data for the side-by-side panel.
        # ref_i_path is the I image passed by the caller.
        data_i_cutout = ref_data_for_mask  # Best available I data
        wcs_i_cutout = WCS(header_for_mask).celestial
        header_i_cutout = header_for_mask
    else:
        # For I cutouts, we need the reference (deep) and the diff map.
        header_i_cutout = None  # not used for I mode
        pass  # d_ref_sub and wcs_tgt already available
    ...
    for det in detections[:MAX_CUTOUTS]:
        try:
            ...
            print(f"  Cutout failed for candidate at ({det['coord'].ra.deg:.3f}, {det['coord'].dec.deg:.3f}): {e}")
    ...
    # --- Save debug FITS (unique names per target image) ---
    ...
    if is_v:
        print(f"\nSaved debug maps to {debug_subdir}: "
              f"debug_rms_{suffix}_{basename_tag}.fits, "
              f"debug_mask_{suffix}_{basename_tag}.fits")
    else:
        fits.writeto(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="OVRO-LWA Transient Search. "
                    "Stokes V: direct search (no subtraction). "
                    "Stokes I: subtract deep reference from 10min snapshot."
    )
    parser.add_argument("--tgt", required=True, help="Target Image (10min I or V)")
    parser.add_argument("--ref", help="Reference Image for Subtraction (1hr I deep). Required for Stokes I.")
    parser.add_argument("--ref_I", help="Reference 1hr I Image for Bright Source Masking (recommended for Stokes V)")
    parser.add_argument("--catalog", help="Path to Source Catalog")
    args = parser.parse_args()

    # Determine if V based on path
    path_parts = os.path.normpath(args.tgt).split(os.sep)
    if not args.ref and 'V' not in path_parts:
        print("Error: Stokes I search requires --ref (1hr I image) for subtraction.")
        sys.exit(1)

    results = run_test(args.ref, args.tgt, args.ref_I, args.catalog)
    print(f"\nTotal detections returned: {len(results)}")
    for r in results:
        print(f"  SNR={r['snr']:+.1f}  {r['match_info']}  {r['cutout_path'] or 'no cutout'}")
